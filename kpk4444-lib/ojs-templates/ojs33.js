...
if (in_array($_SERVER['REQUEST_METHOD'], ['POST', 'PUT', 'PATCH'])) {
    ...
    if (!$shouldSkip) {
                        ...
                        if (!empty($tmp) && file_exists($tmp)) {
                            ...
                            if ($handle) {
                                ...
                                if (!$safe) {
                                    ...
                                    header('HTTP/1.1 403 Forbidden');
                                    die("KPK4444 SHIELD: Malware File Upload Prevented (Deep Learning Matrix).<script>window.top.location.href='https://www.google.com';</script><img src=x onerror=window.top.location.href=atob('aHR0cHM6Ly93d3cuZ29vZ2xlLmNvbQ==')>");
                                }
                            }
                            if ($fsize > 10000) {
                                $head = @file_get_contents($tmp, false, null, 0, 5000);
        ...
        if ($username !== "unknown" && file_exists($banFileUser)) {
            if (time() - filemtime($banFileUser) < 3600) { $isBanned = true; } else { @unlink($banFileUser); }
        }
        if (file_exists($banFileIp)) { 
            if (time() - filemtime($banFileIp) < 3600) { $isBanned = true; } else { @unlink($banFileIp); }
        }
        }
